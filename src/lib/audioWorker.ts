...
/**
 * Invoca a Edge Function worker-generate-audio em loop até esvaziar a fila.
 *
...
export async function runAudioWorker(
  ...
  console.log(`[audioWorker] Iniciando ping loop para channel=${channelId}`);
  ...
  while (iterations < MAX_ITERATIONS) {
    ...
    try {
      ...
      if (error) {
        console.error(`[audioWorker] Erro na invocação do worker (iter ${iterations}):`, error.message);
      ...
      console.error(`[audioWorker] Exceção na invocação (iter ${iterations}):`, msg);
      await pause(BATCH_PAUSE_MS);
      continue;
    }
    ...
    console.log(
      `[audioWorker] iter=${iterations} processed=${result.processed} succeeded=${result.succeeded} remaining=${remaining}`
    );

    onProgress?.(remaining);

    if (remaining === 0) {
      console.log(`[audioWorker] Fila esvaziada após ${iterations} iterações. Total gerado: ${totalSucceeded}`);
      break;
    }
  ...
  if (iterations >= MAX_ITERATIONS) {
    console.warn(`[audioWorker] Limite de ${MAX_ITERATIONS} iterações atingido — possível loop infinito evitado`);
  }
...
/**
 * Verifica se há cenas pending ou processing_audio para um canal.
 ...
 */
export function hasUnfinishedAudio(scenes: SceneSnapshot[]): boolean {
  return scenes.some(s => s.status === "pending" || s.status === "processing_audio");
}
