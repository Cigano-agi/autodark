...
export function useYouTubeMetrics() {
  ...
  const connectWithApify = useMutation({
    ...
    onSuccess: (data) => {
      ...
      toast.success(`YouTube conectado! ${data.data?.channel_name}: ${(data.data?.subscribers ?? 0).toLocaleString()} inscritos, ${data.data?.videos_imported ?? 0} vídeos importados`);
    },
    onError: (error: Error) => {
  ...
  const syncMetrics = useMutation({
    ...
    onSuccess: (data) => {
      ...
      queryClient.invalidateQueries({ queryKey: ['contents'] });
      queryClient.invalidateQueries({ queryKey: ['channel-metrics'] });
      toast.success(`Métricas atualizadas! ${(data.data?.subscribers ?? 0).toLocaleString()} inscritos, ${(data.data?.monthly_views ?? 0).toLocaleString()} views/mês`);
    },
    onError: (error: Error) => {
