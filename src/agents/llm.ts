...
export async function callAI33Image(prompt: string): Promise<string> {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (error) throw error;
      if (!data?.url) throw new Error(data?.error || "AI33: no image URL returned");
      return data.url as string;
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : String(err);
      if (attempt < maxRetries) {
        console.warn(`[callAI33Image] Falha na tentativa ${attempt}/${maxRetries} (${msg}). Refazendo em ${attempt * 3}s...`);
  ...
  throw new Error("AI33: failed after retries");
}

export async function callImageGeneration(prompt: string): Promise<string> {
  // 1. AI33 SeedDream (server-side, task-based)
  try {
 ...
 * Retorna audioUrl (CDN permanente) + duração estimada
 */
export async function callTTS(text: string, _voice: string, voiceId: string): Promise<{ audioUrl: string; durationSec: number }> {
  ...
  if (!data?.audio_url) {
    throw new Error(data?.error || "Edge Function não retornou audio_url");
  }

  // Estima duração baseado no texto (fallback seguro)
  const durationSec = Math.ceil(text.trim().split(/\s+/).length / 2.5);

  return { audioUrl: data.audio_url, durationSec };
}
